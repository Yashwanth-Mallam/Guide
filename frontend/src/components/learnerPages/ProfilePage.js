  ...
  const fetchProfile = async () => {
    try {
      ...
      if (!response.data || Object.keys(response.data).length === 0) {
        // No profile found, allow user to create a new profile
        setProfile(null);
        setIsNewProfile(true);
      } else {
        // Profile exists, allow user to edit it
...
await createLearnerProfileAPI(profile);
      ...
      console.log(isNewProfile ? "Profile created successfully" : "Profile updated successfully");
    } catch (error) {
  ...
  <div className="max-w-3xl mx-auto bg-white p-8 rounded-lg shadow-lg">
    {isNewProfile ? (
          <form onSubmit={handleSubmit}>
            <h2 className="text-3xl font-semibold text-gray-900 mb-4">Create Profile</h2>
            ...
            {/* Coding Level */}
            <div className="mb-4">
              <label className="block text-gray-700 font-medium mb-2">Coding Level</label>
              <select
                name="codingLevel"
 ...
 : !editing ? (
      <div>
        ...
        <p className="text-lg text-gray-800 mb-4">
          ...
          {profile?.interests?.length > 0 ? profile.interests.join(", ") : "N/A"}
        </p>
        <p className="text-lg text-gray-800 mb-4">
